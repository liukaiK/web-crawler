$(document).ready(
		function() {
			$("#Sbmenu a:last").focus();
			var currentPage = $("#page font input:eq(-2)").val();// 获取当前页面的页数
			$("#page font input:eq(-2)").remove();
			var originalHref = $("#esd_original").attr("href").substring(0,$("#esd_original").attr("href").lastIndexOf("/") + 1);
			var content = $("#page").text();// 获取关于页数的文字,用于获取总页数
			...
			selectHTML += "</select>";
			$("#page font span").append(selectHTML);
			...
			$("#NewsListConter tr").each(function(){
				$(this).find("td:first a:first").remove();;
